...
public class CalendarUtil {
    ...
    public static void main(String[] args) throws Exception{
        ...
        String date = "2020年01月02日";
        CalendarUtil calendarUtil = new CalendarUtil();
        HashMap<String, Object> solarToLunar = calendarUtil.solarToLunar(date, today);
        System.out.println(JSONObject.toJSONString(solarToLunar));
        System.out.println(solarToLunar.get("cY").toString()
                +solarToLunar.get("cM")+solarToLunar.get("cD"));
    }
    ...
    private int getTraditionYearBySolarTerm(int year, int month, int day) {
        ...
        if (month < 2) {
            ...
            //获取当年立春的日子
            if (day >= ((solarTerms[(year - MIN_SOLAR_YEAR) * 12 + 1]) >> 4)) {
                nTradYesrs = year;
            } else {
                nTradYesrs = year - 1;
            }
    ...
    private int getMonthGanIndexBySolarTerm(int year, int month, int day) {
        ...
        //当前月的节气的开始
        if (day >= (solarTerms[(year - MIN_SOLAR_YEAR) * 12 + month - 1] >> 4)) {
