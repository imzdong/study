...
public class PhantomReferDemo {
    ...
    private static final ReferenceQueue<ReferDemo> QUEUE = new ReferenceQueue<>();

    public static void main(String[] args) {
        PhantomReference<ReferDemo> phantomReference = new PhantomReference<>(new ReferDemo(), QUEUE);
        new Thread(() -> {
            while (true) {
        ...
        new Thread(() -> {
            while (true) {
                Reference<? extends ReferDemo> poll = QUEUE.poll();
                if (poll != null) {
